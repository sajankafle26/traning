        ...
        <div className="pt-32">
            <AdminCRUD
                ...
                renderItem={(item, onDelete, onEdit) => (
                    <div key={item._id} className="bg-white border border-slate-200 rounded-[2.5rem] p-8 relative group hover:border-indigo-300 transition-all">
                        <div className="absolute top-6 right-6 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => onEdit(item)} className="text-indigo-600 font-black text-[10px] uppercase tracking-widest hover:text-indigo-500">Edit</button>
                            <button onClick={() => onDelete(item._id)} className="text-red-600 font-black text-[10px] uppercase tracking-widest hover:text-red-500">Delete</button>
                        </div>

                        <div className="flex items-center gap-6 mb-6">
                            <div className="w-16 h-16 bg-slate-100 text-slate-600 rounded-2xl flex items-center justify-center text-2xl">
                                <FaTicketAlt />
                            </div>
                            <div>
                                <h3 className="text-2xl font-black text-slate-900 tracking-widest uppercase">{item.code}</h3>
                        ...
                        <div className="grid grid-cols-2 gap-4 pt-6 border-t border-slate-100">
                            <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-2 text-slate-900 font-bold text-sm">
                                    <FaCalendarDay className="text-indigo-600 text-xs" />
                        ...
                        {!item.isActive && (
                            <div className="absolute inset-0 bg-white/80 backdrop-blur-[2px] rounded-[2.5rem] flex items-center justify-center border-2 border-dashed border-red-500/30">
