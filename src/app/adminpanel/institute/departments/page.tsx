        ...
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">{depts.length} academic departments</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {depts.map((d: any) => (
                        <div key={d._id} className="bg-white border border-slate-200 rounded-2xl p-7 relative group hover:border-slate-300 transition-all">
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEdit(d)} className="p-2 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100"><FaEdit /></button>
                                <button onClick={() => handleDelete(d._id)} className="p-2 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100"><FaTrash /></button>
                            </div>
                            <div className="flex items-center gap-4 mb-4">
                                <div className="w-14 h-14 rounded-2xl bg-purple-50 text-purple-600 flex items-center justify-center font-black text-xl">{d.code?.[0] || 'D'}</div>
                                <div>
                                    <h3 className="text-slate-900 font-black text-base">{d.name}</h3>
                                    <p className="text-purple-600 text-xs font-black">{d.code}</p>
                                </div>
                            </div>
                            {d.description && <p className="text-slate-500 text-xs mb-4 line-clamp-2">{d.description}</p>}
                            <div className="flex gap-4 border-t border-slate-100 pt-4">
                                <div><p className="text-xs font-semibold text-slate-500">Seats</p><p className="text-slate-900 font-black">{d.totalSeats}</p></div>
                                {d.duration && <div><p className="text-xs font-semibold text-slate-500">Duration</p><p className="text-slate-900 font-black">{d.duration}</p></div>}
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {showForm && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-lg">
                        <div className="p-8">
                            <div className="flex items-center justify-between mb-8">
                                <h2 className="text-xl font-black text-slate-900">{editId ? "Edit Department" : "Add Department"}</h2>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div className="flex gap-3 pt-2">
                                    ...
                                    <button type="submit" disabled={saving} className="flex-1 py-3 rounded-xl bg-[#00548B] text-white font-black text-sm hover:bg-[#004381] transition-all disabled:opacity-60">{saving ? "Saving..." : editId ? "Save Changes" : "Add Department"}</button>
                                </div>
                            </form>
                        </div>
                    </div>
