...
const StudentsPage = () => {
    ...
    const statusColor = (status: string) => {
        ...
        if (status === "Graduated") return "text-blue-700 bg-blue-50 border-blue-200";
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="flex bg-white border border-slate-200 rounded-xl p-1">
                    {["All", ...STATUSES].map(s => (
                        <button key={s} onClick={() => setFilterStatus(s)}
                            className={`px-4 py-2 rounded-lg text-xs font-semibold transition-all ${filterStatus === s ? "bg-[#00548B] text-white" : "text-slate-500 hover:text-slate-900"}`}>
                            {s}
                        </button>
                    ))}
                </div>
                <div className="flex bg-white border border-slate-200 rounded-xl p-1">
                    <select value={filterGroup} onChange={e => setFilterGroup(e.target.value)}
                        className="bg-transparent text-xs font-semibold text-slate-500 focus:outline-none px-4 py-2 cursor-pointer">
            ...
            {/* Table */}
            {loading ? (
                <div className="space-y-3">{[...Array(5)].map((_, i) => <div key={i} className="h-16 bg-slate-100 rounded-2xl animate-pulse" />)}</div>
            ) : filtered.length === 0 ? (
                <div className="bg-white border border-slate-200 rounded-2xl py-24 text-center">
                    <FaUserGraduate className="text-5xl text-slate-300 mx-auto mb-4" />
                    ...
                    <table className="w-full">
                        <thead className="border-b border-slate-200">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 hidden md:table-cell">Roll No</th>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 hidden lg:table-cell">Group</th>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 hidden lg:table-cell">Phone</th>
                                ...
                                <tr key={s._id} className="hover:bg-slate-50 transition-colors group">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-sm uppercase flex-shrink-0">
                                                {s.name?.[0] || 'S'}
                                            </div>
                                            <div>
                                    ...
                                    <td className="px-6 py-4 text-slate-500 text-sm font-bold hidden md:table-cell">{s.rollNo || "—"}</td>
                                    <td className="px-6 py-4 text-[#00548B] text-xs font-bold hidden lg:table-cell">{s.group?.name || "—"}</td>
                                    <td className="px-6 py-4 text-slate-500 text-sm hidden lg:table-cell">{s.phone || "—"}</td>
                                    <td className="px-6 py-4">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => handleEdit(s)} className="p-2 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition-colors"><FaEdit /></button>
                                            <button onClick={() => handleDelete(s._id)} className="p-2 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 transition-colors"><FaTrash /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
                        <div className="p-8">
                            <div className="flex items-center justify-between mb-8">
                                <h2 className="text-xl font-black text-slate-900">{editId ? "Edit Student" : "Add New Student"}</h2>
                                <button onClick={() => { setShowForm(false); setEditId(null); }}
                                    className="w-10 h-10 rounded-xl bg-slate-100 text-slate-500 hover:text-slate-900 flex items-center justify-center transition-colors"><FaTimes /></button>
                            </div>
                            <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 block mb-2">Gender</label>
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-[#00548B] block mb-2">Institute Group</label>
                                    <select value={form.group} onChange={e => setForm(p => ({ ...p, group: e.target.value }))}
                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-[#00548B]/20 focus:border-[#00548B] font-bold">
                                ...
                                <div className="sm:col-span-2">
                                    ...
                                    <textarea value={form.address} onChange={e => setForm(p => ({ ...p, address: e.target.value }))} rows={2}
                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-[#00548B]/20 focus:border-[#00548B] resize-none" />
                                </div>

                                {/* Course Enrollment */}
                                <div className="sm:col-span-2 border-t border-slate-200 pt-6 mt-2">
                                    <label className="text-xs font-semibold text-[#00548B] block mb-4 flex items-center gap-2">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                        {courses.map(course => (
                                            <label key={course._id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer ${
                                                ...
                                                ? "bg-[#00548B]/10 border-[#00548B]/50 text-slate-900" 
                                                ...
                                                {form.enrolledCourses.includes(course._id) && (
                                                    <span className="text-[8px] font-black uppercase tracking-widest bg-[#00548B] text-white px-2 py-0.5 rounded shadow-lg shadow-[#00548B]/20">Enrolled</span>
                                                )}
                                            </label>
                                        ))}
                                    </div>
