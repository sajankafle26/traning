        ...
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">{classes.length} active classes</p>
                </div>
            ...
            {loading ? (
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{[...Array(4)].map((_, i) => <div key={i} className="h-40 bg-slate-100 rounded-2xl animate-pulse" />)}</div>
            ) : classes.length === 0 ? (
                <div className="bg-white border border-slate-200 rounded-2xl py-24 text-center">
                    <FaDoorOpen className="text-5xl text-slate-300 mx-auto mb-4" />
                    <p className="text-slate-500 font-bold">No classrooms yet</p>
                </div>
            ) : (
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {classes.map((c: any) => (
                        <div key={c._id} className="bg-white border border-slate-200 rounded-2xl p-7 relative group hover:border-slate-300 transition-all">
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEdit(c)} className="p-2 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100"><FaEdit /></button>
                                <button onClick={() => handleDelete(c._id)} className="p-2 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100"><FaTrash /></button>
                            </div>
                            <div className="w-12 h-12 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center text-xl mb-4"><FaDoorOpen /></div>
                            <h3 className="text-slate-900 font-black text-base mb-1">{c.name}</h3>
                            {c.room && <p className="text-amber-600 text-xs font-black mb-2">Room: {c.room}</p>}
                            {(c.startTime || c.endTime) && <p className="text-slate-500 text-xs mb-3">{c.startTime} – {c.endTime}</p>}
                            {c.days?.length > 0 && (
                                <div className="flex flex-wrap gap-1 mb-3">
                                    {c.days.map((d: string) => <span key={d} className="text-[9px] font-black bg-slate-100 text-slate-500 px-2 py-1 rounded-lg">{d.slice(0,3)}</span>)}
                                </div>
                            )}
                            <div className="flex items-center gap-2 border-t border-slate-100 pt-3 mt-2">
                                ...
                                <span className="text-slate-900 font-black text-sm">{c.capacity}</span>
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        <div className="p-8">
                            <div className="flex items-center justify-between mb-8">
                                <h2 className="text-xl font-black text-slate-900">{editId ? "Edit Classroom" : "Add Classroom"}</h2>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                <div>
                                    <label className="text-xs font-semibold text-slate-500 block mb-2">Class Days</label>
                                    <div className="flex flex-wrap gap-2">
                                        {DAYS.map(d => (
                                            <button key={d} type="button" onClick={() => toggleDay(d)}
                                                className={`px-3 py-1.5 rounded-lg text-xs font-black transition-all ${form.days.includes(d) ? "bg-[#00548B] text-white" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}>
                                                {d.slice(0, 3)}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="flex gap-3 pt-2">
                                    ...
                                    <button type="submit" disabled={saving} className="flex-1 py-3 rounded-xl bg-[#00548B] text-white font-black text-sm hover:bg-[#004381] transition-all disabled:opacity-60">{saving ? "Saving..." : editId ? "Save Changes" : "Add Classroom"}</button>
                                </div>
                            </form>
                        </div>
                    </div>
